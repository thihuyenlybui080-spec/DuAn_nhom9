...
import org.example.loginregister.server.util.auto_bidding.AutoBidAgent;
...
public class Bidder extends User  {
    ...
    //=====AUTO BIDDING====
    public void enableAutoBid(Auction auction,double Increment,double MaxBid) {
        // Dừng agent cũ nếu đã tồn tại cho phiên này
        AutoBidAgent existing = agents.get(auction.getId());
        if (existing != null) {
            existing.stop();
        }
        AutoBidAgent agent = new AutoBidAgent(this, auction,Increment,MaxBid);
