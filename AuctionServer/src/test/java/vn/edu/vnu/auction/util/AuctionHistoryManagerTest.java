 ...
 * <p>
 * Đảm bảo các chức năng quản lý, lưu trữ và cập nhật trạng thái của lịch sử đấu giá hoạt động chuẩn xác.
 * </p>
 */
class AuctionHistoryManagerTest {
    ...
    void testGetInstance() {
        ...
        assertSame(instance1, instance2, "Hệ thống chỉ được phép tồn tại duy nhất một đối tượng AuctionHistoryManager");
    }
    ...
    void testSaveAndGetResult() {
        ...
        assertEquals(101, retrievedResult.getAuctionId(), "ID của kết quả phải khớp với dữ liệu đã lưu");
    }

    /**
     * Kiểm tra chức năng lưu kết quả sẽ bỏ qua nếu dữ liệu truyền vào là null (tránh NullPointerException).
     */
    @Test
    ...
    void testGetResultsByWinner() {
        ...
        assertEquals(1, filteredResults.size(), "Chỉ có 1 phiên đấu giá thuộc về người chiến thắng này");
        assertTrue(filteredResults.contains(result1), "Danh sách lọc phải chứa đúng phiên đấu giá của người chiến thắng");
    }
    ...
    void testClearHistory() {
        ...
        manager.clearHistory();
        assertTrue(manager.getAllResults().isEmpty(), "Danh sách phải trống rỗng sau khi gọi hàm clearHistory()");
    }
}