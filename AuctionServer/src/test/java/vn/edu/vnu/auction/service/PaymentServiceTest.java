 ...
 * <p>
 * Đảm bảo logic xử lý thanh toán và lập lịch hạn chót thanh toán (deadline)
 * hoạt động chính xác, bao gồm cả việc tương tác với Database.
 * </p>
 */
class PaymentServiceTest {
    ...
    void testGetInstance() {
        ...
        assertSame(instance1, instance2, "Hệ thống chỉ được phép tồn tại duy nhất một đối tượng PaymentService.");
    }

    /**
     * Kiểm tra việc lập lịch hủy phiên nếu người dùng không thanh toán.
     * Sử dụng ArgumentCaptor để lấy tác vụ (Runnable) ra và chạy thử để kiểm chứng logic bên trong.
     */
    @Test
    void testSchedulePaymentDeadline_ExecutesCancellation() throws Exception {
        ...
        Mockito.verify(mockScheduler).schedule(taskCaptor.capture(), Mockito.eq(86400L), Mockito.eq(TimeUnit.SECONDS));
        ...
        try (MockedStatic<AuctionDAO> mockedDao = Mockito.mockStatic(AuctionDAO.class)) {
            ...
            mockedDao.verify(() -> AuctionDAO.updateAuctionStatus(200, AuctionStatus.CANCELED), Mockito.times(1));
        }
    }
    ...
    void testProcessPayment_NotWinner() {
        ...
        assertFalse(success, "Phải trả về false nếu người yêu cầu thanh toán không phải người chiến thắng.");
    }

    /**
     * Kiểm tra chức năng thanh toán sẽ trả về true ngay lập tức nếu phiên này đã được thanh toán (PAID) trước đó.
     */
    @Test
    ...
    /**
     * Kiểm tra chức năng thanh toán sẽ chặn hành vi trả tiền khi phiên đấu giá chưa thực sự kết thúc.
     */
    @Test
    ...
    void testProcessPayment_Success() {
        ...
        try (MockedStatic<AuctionDAO> mockedDao = Mockito.mockStatic(AuctionDAO.class)) {
            ...
            mockedDao.verify(() -> AuctionDAO.updateAuctionStatus(100, AuctionStatus.PAID), Mockito.times(1));
