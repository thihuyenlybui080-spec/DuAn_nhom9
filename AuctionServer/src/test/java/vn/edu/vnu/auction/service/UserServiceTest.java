 ...
 * <p>
 * Đồng bộ cấu trúc kiểm thử theo chuẩn chung: sử dụng MockedStatic cho tầng DAO
 * và Reflection để giả lập các Service phụ thuộc.
 * </p>
 */
class UserServiceTest {
    ...
    void testApplyStatusSideEffects_SellerHasRunningAndOpenAuctions() throws Exception {
        ...
        java.lang.reflect.Field auctionServiceField = UserService.class.getDeclaredField("auctionService");
        auctionServiceField.setAccessible(true);
        auctionServiceField.set(userService, mockAuctionService);

        // 5. Mock tĩnh UserDAO để KHÔNG bị lỗi kết nối Database khi chạy Test
        try (org.mockito.MockedStatic<vn.edu.vnu.auction.dao.UserDAO> mockedUserDAO = org.mockito.Mockito.mockStatic(vn.edu.vnu.auction.dao.UserDAO.class)) {
            ...
            // KIỂM CHỨNG
            org.mockito.Mockito.verify(mockAuctionService, org.mockito.Mockito.times(1)).cancelAuction(101);
            org.mockito.Mockito.verify(mockAuctionService, org.mockito.Mockito.times(1)).removeAuction(102);

            // Xác nhận thêm là DB đã nhận được lệnh Update
            mockedUserDAO.verify(() -> vn.edu.vnu.auction.dao.UserDAO.updateUserStatus(1, UserStatus.BANNED), org.mockito.Mockito.times(1));
        }
    }
}