 ...
 * <p>
 * Kiểm thử khả năng giao tiếp mạng (Socket) và định tuyến chính xác các logic nghiệp vụ
 * xử lý Request/Response của hệ thống.
 * </p>
 */
class ClientHandlerTest {
    ...
    void tearDown() throws IOException, InterruptedException {
        ...
        if (handlerSocket != null) handlerSocket.close();
    ...
    void testHandleUnknownAction() throws Exception {
        ...
        assertTrue(response.getMessage().contains("Unknow action"), "Error must report unknown action.");
    }
    ...
    void testHandleLogin_Success() throws Exception {
        ...
        try (MockedStatic<UserDAO> mockedDao = Mockito.mockStatic(UserDAO.class)) {
            mockedDao.when(() -> UserDAO.getUserByCredentials("testuser", "pass123")).thenReturn(mockUser);
    ...
    void testHandleLogin_AccountLocked() throws Exception {
        ...
        try (MockedStatic<UserDAO> mockedDao = Mockito.mockStatic(UserDAO.class)) {
            ...
            assertEquals("This account has been locked", res.getMessage(), "Must have account locked message.");
        }
    }
    ...
    void testHandleRegister_DuplicateUsername() throws Exception {
        ...
        try (MockedStatic<UserDAO> mockedDao = Mockito.mockStatic(UserDAO.class)) {
            mockedDao.when(() -> UserDAO.registerUser(Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any()))
                    .thenThrow(new DuplicateUsernameException("User exists"));

            Response res = invokeHandleRequest(req);

            assertFalse(res.isSuccess(), "Registration must fail.");
            assertEquals("Username already exists", res.getMessage(), "Must report duplicate username error.");
        }
    }
    ...
    void testHandleGetAuctions() throws Exception {
        ...
        try (MockedStatic<AuctionService> mockedServiceStatic = Mockito.mockStatic(AuctionService.class)) {
