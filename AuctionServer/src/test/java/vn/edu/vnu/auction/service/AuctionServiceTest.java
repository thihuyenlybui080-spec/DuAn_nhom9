 ...
 * <p>
 * Kiểm tra các chức năng quản lý vòng đời của một phiên đấu giá bao gồm:
 * bắt đầu, kết thúc, hủy bỏ, các hàm truy vấn thống kê và khả năng đồng bộ với cơ sở dữ liệu.
 * </p>
 */
class AuctionServiceTest {
    ...
    void testCancelAuction_InMemory() {
        ...
        try (MockedStatic<AuctionDAO> mockedDao = Mockito.mockStatic(AuctionDAO.class)) {
            ...
            mockedDao.verify(() -> AuctionDAO.updateAuctionStatus(auctionId, AuctionStatus.CANCELED), Mockito.times(1));
    ...
    void testEndAuction_Forced() {
        ...
        try (MockedStatic<AuctionDAO> mockedDao = Mockito.mockStatic(AuctionDAO.class)) {
            ...
            mockedDao.verify(() -> AuctionDAO.updateAuctionStatus(auctionId, AuctionStatus.FINISHED), Mockito.times(1));
            Mockito.verify(mockAuctionManager, Mockito.times(1)).removeActive(auctionId);
        }
    }
    ...
    void testGetAuction_FallbackToDB() {
        ...
        try (MockedStatic<AuctionDAO> mockedDao = Mockito.mockStatic(AuctionDAO.class)) {
            ...
            Mockito.verify(mockAuctionManager, Mockito.times(1)).putActive(dbAuction);
        }
    }
    ...
    void testGetAllAuctions() {
        ...
        try (MockedStatic<UserDAO> mockedUserDao = Mockito.mockStatic(UserDAO.class);
            ...
            mockedAuctionDao.when(() -> AuctionDAO.getAllAuctions(mockUsers)).thenReturn(expectedAuctions);

            List<Auction> actualAuctions = auctionService.getAllAuctions();
            assertSame(expectedAuctions, actualAuctions, "Returned list must match data from DAO.");
        }
    }
    ...
    void testGetAuctionsBySeller() {
        ...
        try (MockedStatic<UserDAO> mockedUserDao = Mockito.mockStatic(UserDAO.class);
            ...
            mockedUserDao.when(UserDAO::getAllUsers).thenReturn(mockUsers);
            mockedAuctionDao.when(() -> AuctionDAO.getAuctionsBySeller(sellerId, mockUsers)).thenReturn(expectedAuctions);

            List<Auction> actualAuctions = auctionService.getAuctionsBySeller(sellerId);
            assertSame(expectedAuctions, actualAuctions, "Returned list must match seller data from DAO.");
        }
    }
    ...
    void testGetWonAuctions() {
        ...
        try (MockedStatic<UserDAO> mockedUserDao = Mockito.mockStatic(UserDAO.class);
            ...
            mockedUserDao.when(UserDAO::getAllUsers).thenReturn(mockUsers);
            mockedAuctionDao.when(() -> AuctionDAO.getWonAuctionsByBidder(bidderId, mockUsers)).thenReturn(dbAuctions);
    ...
    /**
     * Kiểm tra chức năng lấy danh sách các phiên đang chạy (getActiveAuctions)
     * và logic lập lịch lại (reschedule) dựa trên thời gian.
     */
    @Test
    void testGetActiveAuctions_ReschedulesCorrectly() {
        ...
        Mockito.when(mockItem.getEndTime()).thenReturn(LocalDateTime.now().plusHours(1)); // Ends in 1 hour
