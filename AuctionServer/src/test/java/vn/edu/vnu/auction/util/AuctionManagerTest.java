 ...
 * <p>
 * Đảm bảo các chức năng quản lý trạng thái phiên đấu giá (thêm, xóa, lấy danh sách)
 * và cơ chế Singleton hoạt động chính xác.
 * </p>
 */
class AuctionManagerTest {
    ...
    /**
     * Kiểm tra xem phương thức getInstance có trả về đúng một đối tượng duy nhất (Singleton) hay không.
     */
    @Test
    ...
    void testPutAndGetActive() {
        ...
        assertEquals(10, retrievedAuction.getId(), "ID của phiên đấu giá lấy ra phải khớp với ID đã lưu");
    }

    /**
     * Kiểm tra chức năng lưu phiên đấu giá sẽ bỏ qua các đối tượng có ID không hợp lệ (nhỏ hơn hoặc bằng 0).
     */
    @Test
    void testPutActive_IgnoresInvalidId() {
        ...
        assertNull(manager.getActive(0), "Các phiên đấu giá có ID <= 0 không được phép đăng ký vào bộ nhớ");
    }
    ...
    void testRemoveActive() {
        ...
        assertNull(manager.getActive(99), "Phiên đấu giá phải bị xóa khỏi bộ nhớ sau khi gọi removeActive()");
    }
    ...
    void testShutdownScheduler() {
        ...
        assertTrue(scheduler.isShutdown(), "Scheduler phải bị tắt hoàn toàn sau khi gọi hàm shutdown()");
    }
}