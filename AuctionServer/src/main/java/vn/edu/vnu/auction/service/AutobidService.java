 ...
 * <p>
 * Lớp này chịu trách nhiệm quản lý và xử lý các yêu cầu auto-bid từ người tham gia đấu giá. Nó duy
 * trì hàng đợi ưu tiên cho mỗi phiên đấu giá và tự động đặt giá khi có người khác đặt giá cao hơn,
 * miễn là giá không vượt quá mức giá tối đa đã thiết lập.
 * </p>
 */
public class AutobidService implements Observer {
  ...
  public void processQueue(int auctionId) {
    ...
    for (AutoBid autoBid : queue) {
      ...
      if (isBidderLocked(autoBid, auctionId)) {
        continue;
      }
      if (currentHighest != null && autoBid.getBidder().getId() == currentHighest.getId()) {
        logger.debug("Skipping auto-bid for bidder {} - already highest bidder",
  ...
  public void enableAutoBid(Bidder bidder, Auction auction, AutoBidConfig config) {
    ...
    if (alreadyLeading) {
      logger.info("Autobid registered but bidder {} already leading - skipping initial bid",
          bidder.getName());
      return;
    }
    double currentPrice = auction.getCurrentPrice();
  ...
  private void placeInitialAutoBid(double nextBid, AutoBidConfig config, Bidder bidder,
      Auction auction) {
