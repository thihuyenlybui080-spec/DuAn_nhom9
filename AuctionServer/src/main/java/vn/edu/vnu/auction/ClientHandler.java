...
import vn.edu.vnu.auction.service.AutobidService;
import vn.edu.vnu.auction.service.BidService;
import vn.edu.vnu.auction.service.ItemService;
import vn.edu.vnu.auction.service.PaymentService;
import vn.edu.vnu.auction.service.UserService;
...
public class ClientHandler implements Runnable {
  ...
  /**
   * chu trình: đọc Request -> xử lý -> Gửi Response. chạy liên tục đến khi Client ngắt kết nối
   */
  @Override
  public void run() {
    try {
  ...
  private Response handleRegister(Request request) {
    try {
      ...
      User user = UserDAO.registerUser(username, password, fullName, email, gender, phoneNumber,
          role);
  ...
  private Response handleGetAuctions(Request request) {
    try {
      ...
      logger.info("handleGetAuctions: Retrieved {} auctions",
          auctions != null ? auctions.size() : 0);
    ...
    try {
      ...
      if (!(loggedInUser instanceof Bidder)) {
        return Response.error(
            "only bidders can place bids. Current user type: " + loggedInUser.getClass()
                .getSimpleName());
      }
      ...
      if (!auction.getItem().getEndTime().equals(endTimeBefore)) {
        ...
        logger.info("Anti_snipe broadcast: auction {} extended to {}", auctionId,
  ...
  private Response handlePayAuction(Request request) {
    try {
      ...
      boolean success = PaymentService.getInstance()
          .processPayment((Bidder) loggedInUser, auctionId);
      if (!success) {
  ...
  private Response handleEnableAutoBid(Request request) {
    try {
      ...
      logger.info("handleEnableAutoBid: auctionId={}, bidderId={}, maxBid={}, increment={}",
      ...
      logger.info("Calling enableAutoBid for bidder: {}, user object: {}", loggedInUser.getName(),
          loggedInUser.getClass().getName());
   ...
   *
   * @param response phản hồi từ server synchronized để tránh hai thread gửi cùng một lúc làm nhầm
   *                 dữ liệu
   */
  private void sendResponse(Response response) {
    try {
      ...
      logger.error("Failed to send response - client may have disconnected or network error: {}",
          e.getMessage());
    }
  }

  /**
   * Dọn dẹp tài nguyên khi client ngắt kết nối. Đóng luồng đầu vào/ra và socket.
   */
