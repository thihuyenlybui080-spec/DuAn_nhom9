 ...
 * <p>
 * Lớp này quản lý trạng thái đấu giá bao gồm đặt giá, thời gian, và thông báo observer. Nó
 * implement interface Subject để thông báo cho observer về cập nhật giá. Các thao tác thread-safe
 * được đảm bảo bằng ReentrantLock.
 * </p>
 */
public class Auction implements Subject, Serializable {
  ...
  /**
   * ReentrantLock thay thế cho synchronized: - Cho phép tryLock() với timeout khi cần thiết. - Hỗ
   * trợ fairness (true) để tránh thread starvation. - Dễ test/mock hơn synchronized.
   */
  ...
  /**
   * Bid list chỉ được ghi bên trong lock nhưng serialize ghi ngoài lock nên dùng
  ...
  /**
   * Public entry point: wraps placeBid trong try/catch. Trả về true nếu đấu thầu thành công, false
   * nếu thất bại.
   */
  ...
  /**
   * Thread-safe với ReentrantLock. Mọi thay đổi trạng thái auction đều nằm trong lock.
   */
  ...
  /**
   * Chỉ dùng nội bộ (Admin.cancelAuction). Dùng lock để đảm bảo an toàn khi set status.
   */
  ...
  /**
   * xóa 1 bidder ,không cho tham gia đấu giá nữa khi đó sẽ cập nhật lại giá và người trả giá cao
   * nhất sau đó thông báo cho mn
   *
   */
  ...
  /**
   * Add multiple bids to the auction (used when loading from database). Thread-safe: acquires lock
