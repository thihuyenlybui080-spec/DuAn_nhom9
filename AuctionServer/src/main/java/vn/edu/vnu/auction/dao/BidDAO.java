...
public class BidDAO {
  ...
  public static boolean insertBid(int auctionDbId, int bidderId, double amount) {
    ...
    try {
      ...
      String lockSql = "SELECT current_price FROM auctions WHERE id = ?";
      double currentPrice;
      ...
      if (amount <= currentPrice) {
        conn.rollback();
        System.err.println(
            "[BidDAO] insertBid: Bid amount " + amount + " not greater than current price "
                + currentPrice);
        return false;
      }
      ...
      if (insertRows == 0 || updateRows == 0) {
        conn.rollback();
        logger.error("[BidDAO] insertBid: No rows updated - insertRows={}, updateRows={}",
            insertRows, updateRows);
        return false;
      }

      conn.commit();
      logger.info("[BidDAO] insertBid: SUCCESS - auctionDbId={}, bidderId={}, amount={}",
          auctionDbId, bidderId, amount);
      return true;
  ...
  public static List<BidTransaction> getBidHistory(int bidderId, Bidder bidder) {
    ...
    try (Connection conn = DatabaseConfig.getConnection();
      ...
      logger.info("[BidDAO] getBidHistory: Retrieved {} bids for bidderId={}", list.size(),
          bidderId);
    } catch (SQLException e) {
   ...
   */
  public static void insertBidTransaction(int auctionDbId, int bidderId, int itemDbId,
      double finalAmount) {
