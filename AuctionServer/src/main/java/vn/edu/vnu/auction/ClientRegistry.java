 ...
 *
 * <p>
 * khi client bắt đầu xem phiên -> đăng kí vào registry. khi có bid mới -> server dùng registry để
 * notify đúng client
 * <pre>
...
public class ClientRegistry {
  ...
  public void register(int auctionId, ObjectOutputStream stream) {
    registry.computeIfAbsent(auctionId, k -> ConcurrentHashMap.newKeySet())
        .add(stream);
    logger.info("Client registered for auction: {} (total: {} )", auctionId,
        registry.get(auctionId).size());
  }
