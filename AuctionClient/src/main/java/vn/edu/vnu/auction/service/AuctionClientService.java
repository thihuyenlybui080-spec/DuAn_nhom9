...
public class AuctionClientService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(
      AuctionClientService.class.getName());
  ...
  /**
   * Tạo Stripe payment link cho phiên đấu giá đã thắng. Server trả về URL → client mở trình duyệt
   * để thanh toán.
   *
   * @param auctionId id phiên đấu giá
  ...
  /**
   * Gửi Request lên Server và đăng kí đợi response trong 10 giây đăng kí, chờ nhận, hủy đăng kí
   *
  ...
  private synchronized Response sendRequest(Request request) {
    ...
    logger.info("Sending request [{}]: action={}, data={}", request.getRequestId(),
        request.getAction(), request.getData());
    LinkedBlockingQueue<Response> queue = MessageRouter.getInstance()
        .registerRequest(request.getRequestId());
    try {
      ...
      if (!request.getRequestId().equals(response.getRequestId())) {
        logger.error("REQUEST ID MISMATCH! sent={} received={}", request.getRequestId(),
  ...
  /**
   * Báo server biết client đang xem phiên này -> nhận thông báo gọi khi mở màn BiddingController
   *
   * @param auctionId id của phiên đấu giá
   */
